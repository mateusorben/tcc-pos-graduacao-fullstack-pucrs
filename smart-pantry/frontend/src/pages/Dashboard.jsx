...
export default function Dashboard() {
    ...
    async function handleCardClick(filter, title) {
        setModalTitle(title);
        setIsModalOpen(true);
        ...
        try {
            ...
            if (filter === 'shopping-list') {
                data = await ProductService.getShoppingList();
            } else {

                data = await ProductService.getAll(filter);
            }
            setModalItems(data);
        } catch (error) {
            ...
            <main className="max-w-6xl mx-auto p-6 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    < KpiCard
                        ...
                        onClick={() => handleCardClick(null, "Todos os Produtos")}
                    />
                    <KpiCard
                        ...
                        icon={<TrendingUp />}
                        color="bg-amber-500"
                        onClick={() => handleCardClick('expiring', "Vencem em Breve")}
                    />
                    <KpiCard
                        title="Já Vencidos"
                        ...
                        icon={<AlertTriangle />}
                        color="bg-rose-500"
                        onClick={() => handleCardClick('expired', "Produtos Vencidos")}
                    />
                    <KpiCard
                        ...
                        icon={<ShoppingCart />}
                        color="bg-blue-500"
                        onClick={() => handleCardClick('shopping-list', "Lista de Compras Sugerida")}
                    />
                </div>
...
function KpiCard({ title, value, icon, color, onClick }) {
    return (
        <div
            onClick={onClick}
            className={`bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 flex items-center justify-between hover:scale-105 transition-transform cursor-pointer active:scale-95`}
        >
            <div>
